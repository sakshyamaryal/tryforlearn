...
function send_email($to_email, $subject, $message)
{
    ...
    if ($ci->email->send()) {
        ...
        echo 'Please Contact Server Administration';
        exit;
        //return false;
...
function APIKEY()
{
    ...
    if(isset($header['Apikey']) || isset($header['apikey'])) {
        // Normalize header key to lowercase
        $headerKey = isset($header['Apikey']) ? $header['Apikey'] : (isset($header['apikey']) ? $header['apikey'] : null);
    } else {
        $headerKey = null;
    }
    ...
    // Validate API key
    if (trim($headerKey) == $expected_api_key) {
        return true;
    } else {
        header('Content-Type: application/json');
        http_response_code(401);
