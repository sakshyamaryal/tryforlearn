<script>
	...
	$(document).ready(function () {
		dataSource = new kendo.data.DataSource({
			transport: {
				read: "<?php echo base_url(); ?>clas/get_class",
				update: {
				...
				create: {
					url: "<?php echo base_url(); ?>clas/add",
					complete: function (e) {
						toastr.success('class has been added', {timeOut: 5000})
						$("#grid").data("kendoGrid").dataSource.read();
					}
				},
				upload: {
				...
				destroy: {
					...
					complete: function (e) {
						toastr.success('class has been deleted', {timeOut: 5000})
						$("#grid").data("kendoGrid").dataSource.read();
					}
				},
			...
			columns: [
				{
					...
					template: function (dataItem) {
						return `<input type='checkbox' class='rowCheckbox' data-id='${dataItem.classid}' /> ${++record}`;
					},
					width: "50px",
					filterable: false
				},
		...
		$("#delete").on("click", function name(e) {
			...
			selectedRows.each(function () {
				...
				selectedData.push(dataItem.classid);
			});
