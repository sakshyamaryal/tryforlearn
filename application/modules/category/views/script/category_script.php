<script>
	...
	$(document).ready(function () {
		dataSource = new kendo.data.DataSource({
			transport: {
				read: "<?php echo base_url(); ?>category/get_category",
				update: {
			...
			columns: [
				{
					...
					template: function (dataItem) {
						return `<input type='checkbox' class='rowCheckbox' data-id='${dataItem.category_id}' /> ${++record}`;
					},
					width: "50px",
					filterable: false
				},
		...
		$("#delete").on("click", function name(e) {
			...
			selectedRows.each(function () {
				...
				selectedData.push(dataItem.category_id);
			});
