<script>
	...
	$(document).ready(function () {
		dataSource = new kendo.data.DataSource({
			transport: {
				read: "<?php echo base_url(); ?>banner/get_banners",
				update: {
					url: "<?php echo base_url(); ?>banner/update",
					complete: function (e) {
				...
				destroy: {
					...
					complete: function (e) {
						toastr.success('Banner has been deleted', { timeOut: 5000 })
						$("#grid").data("kendoGrid").dataSource.read();
					}
				},
			...
			schema: {
				...
				model: {
					...
					fields: {
						...
						banner_name: { type: "string", validation: { required: true } },
			...
			columns: [
				{
					...
					template: function (dataItem) {
						return `<input type='checkbox' class='rowCheckbox' data-id='${dataItem.banner_id}' /> ${++record}`;
					},
					width: "50px",
					filterable: false
				},
				...
				},
				{
					...
					title: "Image",
					editor: fileUploadEditor,
					template: "<img src='<?php echo base_url(); ?>upload/banner/#= image #' target='_blank' height='50' width='50' class='img-responsive'  >",
					width: "130px",
					filterable: false
		...
		$("#delete").on("click", function name(e) {
			...
			selectedRows.each(function () {
				...
				selectedData.push(dataItem.banner_id);
			});
