...
{ 
 ...
 if(name=="" || org=="" || phone=="" || email=="" || status =="" || citizen=="" || title =="" || doc=="" || cType=="")
 {
 ...
 }else
 {
     if(id!="")
     {
       ...
       url= "<?php echo base_url('certificate/student_save') ?>";
        ...
        type: 'post',                      
        url: url,
        data:{name:name,org:org,phone:phone,email:email,status:status,citizen:citizen,title:title, doc:doc},
        beforeSend: function () {
             $('#loader').show();
        },
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            { 	$('#id').val(res.data.SC_ID);
                 ...
                 $('#certificateType').val(res.data.cType);
