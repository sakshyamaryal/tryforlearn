...
class Qngroup_model extends CI_Model
{
	public function savegroup ($data_arr)
	{
		$this->db->insert('questiongroup', $data_arr);
	...
	public function get_group ()
	{
		...
		$query = $this->db->query("select * from questiongroup where is_active=1 and  $where order by $field $order limit $skip, $take");
		return $query->result();
	}
	...
	public function updategroup ($data, $gid)
	{
		...
		if ($this->db->update('questiongroup', $data)) {
			return true;
		} else {
			return false;
		}
