<script>
	...
	$(document).ready(function () {
		dataSource = new kendo.data.DataSource({
			transport: {
				read: "<?php echo base_url(); ?>events/get_events",
				update: {
					url: "<?php echo base_url(); ?>events/update",
					complete: function (e) {

						toastr.success('Event has been updated', {timeOut: 5000})
						$("#grid").data("kendoGrid").dataSource.read();
					}
				},
				create: {
					url: "<?php echo base_url(); ?>events/add",
					complete: function (e) {
						toastr.success('Event has been added', {timeOut: 5000})
						$("#grid").data("kendoGrid").dataSource.read();
					}
				},
				upload: {
				...
				destroy: {
					...
					complete: function (e) {
						toastr.success('Event has been deleted', {timeOut: 5000})
						$("#grid").data("kendoGrid").dataSource.read();
					}
				},
				...
				parameterMap: function (data, operation) {
					...
					if ('filter' in data) {
						for (var i = 0; i < data.filter.filters.length; i++) {
							if (data.filter.filters[i].field == 'created_at' || data.filter.filters[i].field == 'happening_date') {
			...
			schema: {
				...
				model: {
					...
					happening_at: {type: "string", validation: { required: true}},
					happening_date: {type: "date", validation: { required: true}},
		...
		function dateTimeEditor2(container, options) {
			$('<input type="text" />')
				...
				.kendoDateTimePicker({
					...
					value: kendo.toString(new Date(options.model.happening_date), 'yyyy-MM-dd hh:mm')
				});
		}
