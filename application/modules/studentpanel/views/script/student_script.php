...
function getgrades(id)
{
    ...
    url=base_url+"mygrades/getbytopic";
    var data={chapterid:id};
    $.when(requestmethod(data, url)).then(function(res){
        if(res.status==true)
        {

           $('.tabgradedetail').empty();
           $('.tabgradedetail').html(res.html);
...
$(document).on('click','#getdatasetmodal',function(e){
    e.preventDefault();
    url=base_url+"studentpanel/getdatamodel";
    var data={classid:localStorage.getItem('currentclass'),subjectid:localStorage.getItem('currentsubject')};
    $.when(requestmethod(data, url)).then(function(res){
        if(res.status==true)
        {
