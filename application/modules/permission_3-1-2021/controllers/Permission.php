...
function getclass()
    ...
    function getsubjectdata()
    {
       //,'user_type'=>'7','user_type'=>'8'
        $data=$this->common_model->getRows('users',"is_active=1 and user_type in('7','8','9')",'*','fullname');
        $subject=$this->common_model->getRows('subject',array('is_active'=>1,'classid'=>$_POST['class']),'*','subject_name');
    ...
    {
          $data=$this->common_model->getRows('users',"is_active=1 and user_type in('7','8','9')",'*','fullname');
        $chapter=$this->common_model->getRows('chapter',array('is_active'=>1,'classid'=>$_POST['class'],'subjectid'=>$_POST['subject']),'*','priority');
        ...
        { 
            ...
            $html .= "<tr>
                    ...
                    { if(isset($permission[$res->chapterid."_".$res->subjectid."_".$val->user_id])){
                        if($permission[$res->subjectid."_".$val->user_id]=='1') {
                            $check="checked";
                        }else{
                            $check="";
                        }
