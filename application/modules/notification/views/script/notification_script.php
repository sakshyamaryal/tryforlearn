<script>
	...
	function myImage(model) {
		...
		for(var i = 0; i < str_array.length; i++) {
			str += '<img src="./upload/notification/'+str_array[i]+'" height="50" width="50" >';
		}
		return str;
	}
	$(document).ready(function () {
		dataSource = new kendo.data.DataSource({
			transport: {
				read: "<?php echo base_url(); ?>notification/get_notification",
				update: {
					url: "<?php echo base_url(); ?>notification/update",
					complete: function (e) {
					$("#uploadedFile").val('');			
				...
				create: {
					url: "<?php echo base_url(); ?>notification/add",
					complete: function (e) {
						$("#uploadedFile").val('');
						toastr.success('notification has been added', {timeOut: 5000})
						$("#grid").data("kendoGrid").dataSource.read();
					}
				},
				upload: {
				...
				destroy: {
					url: "<?php echo base_url(); ?>notification/delete",
					complete: function (e) {
						toastr.success('notification has been deleted', {timeOut: 5000})
						$("#grid").data("kendoGrid").dataSource.read();
					}
				},
		...
		$("#grid").kendoGrid({
			...
			columns: [{
...

				{
					...
					//template: "<img src='<?php //echo base_url(); ?>//upload/notification/#= image #' target='_blank' height='50' width='50' class='img-responsive'  >",
					width: "130px",
					filterable: false
		...
		//			multiple: false,
		//			async: {
		//				saveUrl: "<?php //echo base_url(); ?>///notification/image",
		//				removeUrl:  "<?php //echo base_url(); ?>///notification/removeImage",
		//				autoUpload: true,
		//			},
		//			validation: {
		...
		function fileUploadEditor(container, options) {
			...
			$('<input type="file" id="files" name="files" />')
				...
				.kendoUpload({
					async: {
						saveUrl: "<?php echo base_url(); ?>/notification/updloadImage",
						removeUrl: "<?php echo base_url(); ?>/notification/removeImage",
					},
					files: images,
					upload: function (e) {
						e.data = {fileId: options.model.notification_id};
					},
					remove: function (e) {
						e.data = {fileId: options.model.notification_id};
