...
class Studentexercise_model extends CI_Model
{
  function getexerciselist($post)
  {
    ...
    {
      $where .=" and se.chapterid='".$post['chapter']."'";
    }
    ...
    {
      $join="  join studentexam e on se.setid=e.examsetid
      join exercise es on es.eid=e.question_id and created_by=".$this->session->userdata('adminuserid');
    }

    $sql="select s.user_id,s.fullname,s.phone,s.email,t.* from users s 

    left join ( select setid,se.is_subj_obj,case when se.is_subj_obj='S' then 'Subjective' else 'Objective' end as
     examcategory, case when se.isself='Y' then 'Self Practise' else 'Exam' end as examtype, 
     case when setid is null then 'No Record' else 'Submitted' end as status,examdate,
      totalqn,totalmark,obtainedmark,se.studentid,se.totaltimer,se.submitted_time from studentexamset se
    ".$join."
       where se.levelid=? and  se.isself=? 
      ".$where."
