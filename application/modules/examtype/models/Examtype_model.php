...
{
	public function saveexamtype ($data_arr)
	{
		$this->db->insert('examtype', $data_arr);
	...
	public function get_examtype ()
	{
		...
		$query = $this->db->query("select * from examtype where   $where order by $field $order limit $skip, $take");
		return $query->result();
	}
	...
	public function count_all_examtype ()
	{
		$this->db->select(" * ");
	...
	public function updateexamtype ($data, $examtypeId)
	{
		$this->db->where('examtypeid', $examtypeId);
		if ($this->db->update('examtype', $data)) {
			return true;
		} else {
			return false;
		}
