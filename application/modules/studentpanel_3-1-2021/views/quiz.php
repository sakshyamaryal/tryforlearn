...
<span class="qnread<?=$list->eid;?>" style="display:inline-block;">
<?php
...
<li class="select">
<label class="labels" for="answerGroup_0" id="q_answerlabel<?=$sn; ?><?=$i;?><?=$list->eid;?>" onclick="get_visibility(<?=$sn; ?>,<?=$i;?>,<?=$list->eid;?>)">
...
function get_visibility(val,ansid,qid)
{
    ...
    for(a = 1; a <= at; a++) 
   {
    ...
    $('#tick'+val+ansid+qid).attr('style','visibility: true;  margin-top: -32px;');
}
